...
var popup;
/* CC3 rewrite (phase 5): `popup` is write-only legacy (juicy-queenbeet harvest
 * message); the bare declaration must stay so that strict-mode assignment doesn't
 * throw, and the void read keeps noUnusedLocals quiet. Zero runtime effect. */
void popup;
...
M.launch=function()
{
		...
		*/
		M.plants={
			'bakerWheat':{
				...
				onHarvest:function(_x: any, _y: any, age: any)
				{
			...
			'meddleweed':{
				...
				onKill:function(x: any, y: any, age: any)
				{
		...
		M.dropUpgrade=function(upgrade: any, rate: any)
		{
		...
		M.getMuts=function(neighs: any, neighsM: any)
		{
		...
		M.computeBoostPlot=function()
		{
			...
			var effectOn=function(X: any, Y: any, s: any, mult: any)
			{
		...
		M.getPlantDesc=function(me: any)
		{
			var children='';
		...
		M.soilTooltip=function(id: any)
		{
			return function(){
				...
				var str='<div style="padding:8px 4px;min-width:350px;">'+
					...
					):('<div class="icon" style="background:url(img/gardenPlants.webp?v='+Game.version+');float:left;margin-left:-8px;margin-top:-8px;background-position:'+(-me.icon*48)+'px '+(-34*48)+'px;"></div>'+
		...
		M.seedTooltip=function(id: any)
		{
			return function(){
				var me=M.plantsById[id];
				var str='<div style="padding:8px 4px;min-width:400px;">'+
					'<div class="icon" style="background:url(img/gardenPlants.webp?v='+Game.version+');float:left;margin-left:-24px;margin-top:-4px;background-position:'+(-0*48)+'px '+(-me.icon*48)+'px;"></div>'+
					'<div class="icon" style="background:url(img/gardenPlants.webp?v='+Game.version+');float:left;margin-left:-24px;margin-top:-28px;background-position:'+(-4*48)+'px '+(-me.icon*48)+'px;"></div>'+
					'<div style="background:url(img/turnInto.webp);width:20px;height:22px;position:absolute;left:28px;top:24px;z-index:1000;"></div>'+
		...
		M.toolTooltip=function(id: any)
		{
			return function(){
				var me=M.toolsById[id];
				var icon=[me.icon,35];
				var str='<div style="padding:8px 4px;min-width:350px;">'+
					'<div class="icon" style="background:url(img/gardenPlants.webp?v='+Game.version+');float:left;margin-left:-8px;margin-top:-8px;background-position:'+(-icon[0]*48)+'px '+(-icon[1]*48)+'px;"></div>'+
		...
		M.tileTooltip=function(x: any, y: any)
		{
			return function(){
				...
				if (tile[0]==0)
				{
					...
					var str='<div style="padding:8px 4px;min-width:350px;text-align:center;">'+
						'<div class="name">Empty tile</div>'+'<div class="line"></div><div class="description">'+
							...
							(M.plotBoost[y][x]!=[1,1,1] as any?('<small>'+
					...
					var str='<div style="padding:8px 4px;min-width:350px;">'+
						...
						'<div style="text-align:center;">'+
							'<div style="display:inline-block;position:relative;box-shadow:0px 0px 0px 1px #000,0px 0px 0px 1px rgba(255,255,255,0.5) inset,0px -2px 2px 0px rgba(255,255,255,0.5) inset;width:256px;height:6px;background:linear-gradient(to right,#fff 0%,#0f9 '+me.mature+'%,#3c0 '+(me.mature+0.1)+'%,#960 100%)">'+
								...
								'<div style="background:url(img/gardenPlants.webp?v='+Game.version+');background-position:'+(-1*48)+'px '+(-icon[1]*48)+'px;position:absolute;left:'+(0-24)+'px;top:-32px;transform:scale(0.5,0.5);width:48px;height:48px;"></div>'+
								'<div style="background:url(img/gardenPlants.webp?v='+Game.version+');background-position:'+(-2*48)+'px '+(-icon[1]*48)+'px;position:absolute;left:'+((((me.mature*0.333)/100)*256)-24)+'px;top:-32px;transform:scale(0.5,0.5);width:48px;height:48px;"></div>'+
								'<div style="background:url(img/gardenPlants.webp?v='+Game.version+');background-position:'+(-3*48)+'px '+(-icon[1]*48)+'px;position:absolute;left:'+((((me.mature*0.666)/100)*256)-24)+'px;top:-32px;transform:scale(0.5,0.5);width:48px;height:48px;"></div>'+
								'<div style="background:url(img/gardenPlants.webp?v='+Game.version+');background-position:'+(-4*48)+'px '+(-icon[1]*48)+'px;position:absolute;left:'+((((me.mature)/100)*256)-24)+'px;top:-32px;transform:scale(0.5,0.5);width:48px;height:48px;"></div>'+
							'</div><br>'+
		...
		M.buildPanel=function()
		{
			...
			for (var i in M.plants)
			{
				...
				AddEvent(me.l,'click',function(me: any){return function()
				{
			...
			{
				var me=M.soils[i];
				AddEvent(l('gardenSoil-'+me.id),'click',function(me: any){return function(){
		...
		M.buildPlot=function()
		{
			...
			if (!l('gardenTile-0-0'))
			{
				...
				for (var y=0;y<6;y++)
				{
					for (var x=0;x<6;x++)
					{
						AddEvent(l('gardenTile-'+x+'-'+y),'click',function(x: any, y: any){return function()
						{
							M.clickTile(x,y);
						}}(x,y));
					}
		...
		M.clickTile=function(x: any, y: any)
		{
			//if (M.freeze) return false;
		...
		M.useTool=function(what: any, x: any, y: any)
		{
		...
		M.getTile=function(x: any, y: any)
		{
		...
		M.isTileUnlocked=function(x: any, y: any)
		{
		...
		M.harvestAll=function(type: any, mature: any, mortal: any)
		{
			var harvested=0;
		...
		M.harvest=function(x: any, y: any, _manual: any)
		{
			var tile=M.plot[y][x];
			if (tile[0]>=1)
			{
		...
		var str='';
		str+='<style>'+
		'#gardenBG{background:url(img/shadedBorders.webp),url(img/BGgarden.webp);background-size:100% 100%,auto;position:absolute;left:0px;right:0px;top:0px;bottom:16px;}'+
		...
		'.gardenTile:before{transform:translate(0,0);opacity:0.65;transition:opacity 0.2s;pointer-events:none;content:\'\';display:block;position:absolute;left:0px;top:0px;right:0px;bottom:0px;margin:0px;background:url(img/gardenPlots.webp);}'+
		...
		'.gardenTileIcon{transform:translate(0,0);pointer-events:none;transform-origin:50% 40px;width:48px;height:48px;position:absolute;left:-'+((48-M.tileSize)/2)+'px;top:-'+((48-M.tileSize)/2+8)+'px;background:url(img/gardenPlants.webp?v='+Game.version+');}'+
		...
		'#gardenCursor{transition:transform 0.1s;display:none;pointer-events:none;width:48px;height:48px;position:absolute;background:url(img/gardenPlants.webp?v='+Game.version+');}'+
		'.gardenSeed{cursor:pointer;display:inline-block;width:40px;height:40px;position:relative;}'+
		'.gardenSeed.locked{display:none;}'+
		'.gardenSeedIcon{pointer-events:none;transform:translate(0,0);display:inline-block;position:absolute;left:-4px;top:-4px;width:48px;height:48px;background:url(img/gardenPlants.webp?v='+Game.version+');}'+
		...
		'.gardenPanelLabel{font-size:12px;width:100%;padding:2px;margin-top:4px;margin-bottom:-4px;}'+'.gardenSeedTiny{transform:scale(0.5,0.5);margin:-20px -16px;display:inline-block;width:48px;height:48px;background:url(img/gardenPlants.webp?v='+Game.version+');}'+
	...
	M.reset=function(hard: any)
	{
		...
		setTimeout(function(M: any){return function(){M.onResize();}}(M),10);
	}
	M.logic=function()
	{
		//run each frame
							...
							if (tile[0]>0)
							{
								...
								else if (!me.noContam)
								{
									...
									if (contam && me.key!=contam)
									{
										...
										{
											var any=0;
											var neighs: any ={};//all surrounding plants
											var neighsM: any ={};//all surrounding mature plants
