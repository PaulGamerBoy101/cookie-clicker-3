...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		l('versionNumber').innerHTML='v. '+Game.version.toFixed(3)+(!App?('<div id="httpsSwitch" style="cursor:pointer;display:inline-block;background:url(img/'+(Game.https?'lockOn':'lockOff')+'.png);width:16px;height:16px;position:relative;top:4px;left:0px;margin:0px -2px;"></div>'):'')+(Game.beta?' <span style="color:#ff0;">beta</span>':'');
		...
		Game.LoadMinigames=function()//load scripts for each minigame
		{
			for (var i in Game.Objects)
			{
				// CC3: block-scoped (was `var`). The 2.048 original captured `me`
				// by value via an IIFE around the async <script> onload handler;
				// the ES-module import().then() replacement drops that IIFE, so a
				// function-scoped `var me` would let the closure see the loop's
				// final value (the wrong building). `const` gives each iteration
				// its own binding, fixing the capture in the modern idiom.
